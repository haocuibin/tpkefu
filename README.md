...
客服地址:https://www.***.com/service
账户密码：
客服小美 123456 

后台管理地址：https://www.***.com/admin
账户密码：暂不提供

github仓库：https://github.com/haocuibin/tpkefu.git
...
2、如果你是在服务器上运行该项目，请开放7272端口供客服使用，以阿里云为例，在`网络和安全`的`安全组`里面`修改规则`，增加7272端口

3、如果指定某个域名才能connect，那么请修改Events.php的HTTP_ORIGIN参数

4、启动gatawayworker相关服务
如果你是在windows上运行的话，直接根目录双击start_for_bat.bat或双击`/vendor/workerman/Conf/start_for_bat.bat`即可
如果你是在linux或mac上运行的话，直接根目录运行或请进入`/vendor/workerman/Conf/`目录，然后运行
```php
