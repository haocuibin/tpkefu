...
;!function (win) {
    ...
    var Laykefu = function () {
        ...
        if (!getCache('laykefu-UserId')) {//是否已分配账号
            ...
            cacheChat({
                key: "laykefu-UserId",
                data: [{'userId': config.uid, 'name': config.name, 'avatar': config.avatar}]
            });
        }
        ...
        for (var i = 0; i < len; i++) {
            ...
            if ('time' !== v.type) {
                ...
                _html += '<img src="' + imageHost + v.avatar + '">';
                if ('mine' == v.type) {
        ...
        socket.send(JSON.stringify({
            type: 'chatMessage',
            data: {
                to_id: kf_id, to_name: kf_name, content: msg, from_name: uinfo[0].name,
        ...
        if ('time' !== type) {
            ...
            _html += '<div class="laykefu-chat-user">';
            _html += '<img src="' + imageHost + info.avatar + '">';
            if ('mine' == type) {
    ...
    Laykefu.prototype.init = function (options) {
        options.uid = options.uid || '',//客户id      
            ...
            options.avatar = options.avatar || '/uploads/avatar/default.jpg',//用户头像
